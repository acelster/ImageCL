...
int get_n_devices(){
    int n_platforms;
    int* n_devices_per_platform;
    get_n_devices_per_platform(&n_platforms, &n_devices_per_platform);

    int n_total_devices = 0;
    for(int i = 0; i < n_platforms; i++){
        n_total_devices += n_devices_per_platform[i];
    }

    return n_total_devices;
}

void get_n_devices_per_platform(int* n_platforms, int** n_devices_per_platform){
    cl_int error;
    cl_uint n_platforms_cluint;
    
    error = clGetPlatformIDs(0, NULL, &n_platforms_cluint);

    *n_platforms = n_platforms_cluint;
    *n_devices_per_platform = (int*)malloc(sizeof(int)* (*n_platforms));
    
    
    cl_platform_id* all_platforms = (cl_platform_id*)malloc(sizeof(cl_platform_id)*n_platforms_cluint);
    error = clGetPlatformIDs(n_platforms_cluint, all_platforms, NULL);
    
    cl_uint n_devices;
    for(int i = 0; i < *n_platforms; i++){
        error = clGetDeviceIDs(all_platforms[i], CL_DEVICE_TYPE_ALL, 0, NULL, &n_devices);
        (*n_devices_per_platform)[i] = n_devices;
    }
    clError("get_n_devices", error);
}
...
cl_device_id get_device_n(int n){
    ...
    int n_devices_seen = 0;
    for(int i = 0; i < n_platforms; i++){
        for(int j = 0; j < n_devices_per_platform[i]; j++){
            if(n_devices_seen == n){
                return get_device_by_id(i,j);
            }
            n_devices_seen ++;
        }
    }
